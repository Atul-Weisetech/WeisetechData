...
function ManageLeaveRequests() {
  ...
  const [activeFilter, setActiveFilter] = useState('all'); // 'all', 'pending', 'approved', 'declined'
  ...
  const handleStatusUpdate = async (requestId, newStatus) => {
    try {
      ...
      const status = normalized === 'rejected' ? 'declined' : normalized; // align with backend
      const reviewerName =
      ...
      setLeaveRequests(prevRequests => 
        prevRequests.map(request => 
          request.id === requestId 
            ? { 
                ...request, 
                status_text: status.charAt(0).toUpperCase() + status.slice(1),
                status: status
              } 
            : request
        )
      ...
      if (newStatus === 'approved' || newStatus === 'declined') {
        const originalRequest = leaveRequests.find(req => req.id === requestId);
        if (originalRequest?.status_text?.toLowerCase() === 'requested') {
  ...
  const getStatusButtonClass = (status, isSelected) => {
    ...
    if (isSelected) {
      ...
      return status === 'approved'
        ...
        : `${baseClass} bg-red-100 text-red-800 border-red-300 hover:bg-red-200`;
    }
  };
    ...
    <div className="p-4 sm:p-6 overflow-x-hidden max-w-full">
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="min-w-0">
          <h1 className="text-xl sm:text-2xl font-bold text-gray-800 truncate">Manage Leave Requests</h1>
          <p className="text-sm sm:text-base text-gray-600">Review and manage employee leave requests</p>
        </div>
      ...
      <div className="mb-6 overflow-x-hidden">
        <div className="flex gap-1 sm:gap-2 flex-nowrap">
          <button 
            ...
            className={`${getFilterButtonClass('all')} px-1.5 sm:px-4 py-1 sm:py-2 text-[10px] sm:text-sm whitespace-nowrap flex-1 sm:flex-none`}
          >
            All Requests
          </button>
          <button 
            onClick={() => setActiveFilter('pending')}
            className={`${getFilterButtonClass('pending')} px-1.5 sm:px-4 py-1 sm:py-2 text-[10px] sm:text-sm whitespace-nowrap flex-1 sm:flex-none`}
          >
            Pending
          </button>
          <button 
            onClick={() => setActiveFilter('approved')}
            className={`${getFilterButtonClass('approved')} px-1.5 sm:px-4 py-1 sm:py-2 text-[10px] sm:text-sm whitespace-nowrap flex-1 sm:flex-none`}
          >
            Approved
          </button>
          <button 
            onClick={() => setActiveFilter('declined')}
            className={`${getFilterButtonClass('declined')} px-1.5 sm:px-4 py-1 sm:py-2 text-[10px] sm:text-sm whitespace-nowrap flex-1 sm:flex-none`}
          >
            Declined
          </button>
        </div>
      ...
      {filteredRequests.length === 0 ? (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">
            {activeFilter === 'all' ? 'No Leave Requests' : `No ${activeFilter.charAt(0).toUpperCase() + activeFilter.slice(1)} Requests`}
          </h3>
          <p className="text-gray-500">
            {activeFilter === 'all' 
              ? 'There are no leave requests to display.' 
              : `There are no ${activeFilter} leave requests.`}
          </p>
        </div>
      ) : (
                  ...
                  {filteredRequests.map((req) => (
                    <tr key={req.id}>
                      ...
                      <td className="px-2 sm:px-6 py-4 text-xs sm:text-sm text-gray-700 max-w-[120px] sm:max-w-xs">
                        <div className="line-clamp-2 sm:line-clamp-1 truncate" title={req.description}>
                          {req.description}
                        </div>
                      </td>
                      <td className="px-2 sm:px-6 py-4 sm:whitespace-nowrap">
                        ...
                        <div className="flex flex-col gap-2">
                          <div className="flex gap-2">
                            <button
                              ...
                              className={getStatusButtonClass('approved', editingRequest.currentStatus === 'approved')}
                            >
                              Approve
                            </button>
                            <button
                              onClick={() => handleStatusUpdate(req.id, 'declined')}
                              className={getStatusButtonClass('declined', editingRequest.currentStatus === 'declined')}
                            >
                              Decline
                            </button>
                          </div>
                        ...
                        <div className="flex flex-col gap-2">
                          {req.status_text.toLowerCase() === 'requested' ? (
                            ...
                            <div className="flex gap-2 items-center">
                              ...
                              <button
                                onClick={() => handleEditStatus(req.id, req.status_text)}
